...
@click.pass_context
@click.version_option(version="1.0.0", prog_name="TRACE")
def cli(ctx):
    """
    TRACE: Time-series representation, annotation, and computation engine.

    Use 'tracengine gui' to launch the interactive GUI, or 'tracengine pipeline' for
...
def gui(session_path):
    ...
    Examples:
        ...
        tracengine gui ./test-data/ses-01
    """
    from tracengine.gui.main_window import run_trace_tool
...
def pipeline(pipeline_path, session_path, dry_run, run_filter, output, verbose):
    ...
    Examples:
        ...
        tracengine pipeline ./pipelines/default.yaml ./data/ses-01

        tracengine pipeline ./pipelines/default.yaml ./data/ses-01 --dry-run

        tracengine pipeline ./pipelines/default.yaml ./data/ses-01 -f "run-00*"
    """
    from tracengine.project.structure import load_pipeline
    from tracengine.data.loader import load_session
    from tracengine.engine.runner import PipelineRunner
    from tracengine.engine.export import export_results
    from tracengine.project.config import ProjectConfig, ProjectPaths
...
def init(project_path, name):
    """
    Initialize a new TRACE project structure.

    Creates the standard directory layout and configuration files.

    Example:

        tracengine init ./my-project --name "My Research Project"
    """
    from tracengine.project.structure import init_project
    ...
    click.echo("    └── trace-project.yaml  # Project manifest")
    click.echo()
...
def validate(project_path, fix):
    """
    Validate a TRACE project structure.

    Checks for required directories, config files, and data integrity.

    Example:

        tracengine validate ./my-project
    """
    from tracengine.project.structure import validate_project, load_project
    from tracengine.data.loader import load_session
    ...
    # Check trace-project.yaml
    click.echo("Checking project configuration...")
    manifest_path = project_path / "trace-project.yaml"
    if manifest_path.exists():
        ...
        warnings.append("No trace-project.yaml found")
        click.echo("  ⚠ No trace-project.yaml found (optional)")
...
def link_data(project_path, data_path):
    ...
    Example:
        ...
        tracengine link-data ./my-project ./data
    """
...
def provenance(provenance_path):
    ...
    """
    from tracengine.engine.provenance import show_provenance_graph
...
def reset_notebooks(project_path, force):
    ...
    Example:
        ...
        tracengine reset-notebooks ./my-project

        tracengine reset-notebooks ./my-project --force
    """
    import shutil
